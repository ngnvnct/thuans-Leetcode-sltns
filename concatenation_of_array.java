...
Method 1: Simlar to the N-th Tribonacci Number Problem https://leetcode.com/problems/n-th-tribonacci-number/
...
Method 2:  This method will have O(n) time where n is the length of the input array
We loop through the input array, then set current index = current element at input array, then we also set the next set of index (i + nums.length) to be the same
